...
const Comment: FC<CommentProps> = ({ isSmall, review }) => {

  const stars: any = [];

  for (let index = 0; index < Number(review?.rating); index++) {
  ...
  return (
    <div className="nc-CommentCard flex ">
      <div className="pt-1">
        <Avatar sizeClass={`w-6 h-6 ${!isSmall ? "sm:h-8 sm:w-8 " : ""}`} imgUrl={review?.user?.photo ? `${process.env.NEXT_PUBLIC_APP_API_URL}${review?.user.photo}` : avatarImgs[10]} />
      </div>
